...
class SectionRepository implements SectionRepositoryInterface
{
    ...
    {
        cacheRemoveByType(type: 'sections');
        return $this->section->create($data);
    }

    public function getFirstWhere(array $params, array $relations = []): ?Model
    {
        return $this->section->where($params)->with($relations)->first();
    }

    public function getList(array $orderBy = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->section->with($relations)
    ...
    public function getListWhere(array $orderBy = [], string $searchValue = null, array $filters = [], array $relations = [], int|string $dataLimit = DEFAULT_DATA_LIMIT, int $offset = null): Collection|LengthAwarePaginator
    {
        $query = $this->section->with($relations)
            ->when($searchValue, function ($query) use($searchValue){
                return $query->orWhere('section_type', 'like', "%$searchValue%");
            })
            ->when(!empty($orderBy), function ($query) use ($orderBy) {
                return $query->orderBy(array_key_first($orderBy),array_values($orderBy)[0]);
            })
    ...
    {
        cacheRemoveByType(type: 'sections');
        $this->section->find($id)->update($data);
        return true;
    }
    ...
    {
        cacheRemoveByType(type: 'sections');
        $this->section->where($params)->delete();
        return true;
    }
}
